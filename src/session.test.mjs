...
test('다른 용도로 만든 토큰을 세션으로 읽으면 거부한다', () => {
  const 다른토큰 = 세션만들기(사용자, 비밀, 지금, 600, '다른용도');
  assert.equal(세션읽기(다른토큰, 비밀, 지금), null);
  assert.deepEqual(세션읽기(다른토큰, 비밀, 지금, '다른용도'), 사용자);
});

test('세션 토큰을 다른 용도로 읽으면 거부한다', () => {
  const 세션토큰 = 세션만들기(사용자, 비밀, 지금);
  assert.equal(세션읽기(세션토큰, 비밀, 지금, '다른용도'), null);
});
