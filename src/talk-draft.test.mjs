...
test('모델이 성구 문장을 직접 쓰면 한 번 다시 요청한다', async () => {
  ...
  const { 구조체, 생성 } = await 살채우기({
    뼈대: 시연뼈대, 프로필, 매니페스트, 읽기,
    설정: 응답하는([
      { 대사: [{ 구간: '여는 인사', 화자: '전도인', 말: '예레미야 29:11은 “내가 너희에 대해 갖고 있는 생각을 나는 잘 알고 있다. 그것은 평화를 주려는 생각이다.”라고 말합니다.', 동작: '', 요점표시: '' }], 단락: [], 축약순서: [] },
...
test('다시 요청해도 위반이면 그 대사를 비우고 경고를 남긴다', async () => {
  const 나쁜대사 = { 대사: [{ 구간: '여는 인사', 화자: '전도인', 말: '예레미야 29:11은 “내가 너희에 대해 갖고 있는 생각을 나는 잘 알고 있다. 그것은 평화를 주려는 생각이다.”라고 말합니다.', 동작: '', 요점표시: '' }], 단락: [], 축약순서: [] };
...
test('교재에서 온 소제목 문장을 모델이 바꿔도 원본을 지킨다', async () => {
  ...
  const { 구조체 } = await 살채우기({
    ...
    설정: 응답하는({
      대사: [],
      단락: [{ 소제목: '모델이 고친 소제목입니다', 요점: ['요점 하나'], 예화: '예화', 적용: '적용', 성구주소: [], 요점표시: '요점 4를 보인다' }],
      축약순서: [],
    }),
  });
  ...
  assert.equal(구조체.단락[0].요점표시, '요점 4를 보인다');
});
...
test('단락 예화에 성구 문장을 두 번 다 직접 쓰면 그 칸만 비우고 소제목·성구주소는 남긴다', async () => {
  ...
  const 나쁜단락 = {
    대사: [], 축약순서: [],
    단락: [{ 소제목: '무시될 소제목', 요점: [], 예화: '예레미야 29:11은 “내가 너희에 대해 갖고 있는 생각을 나는 잘 알고 있다.”라고 말합니다.', 적용: '', 성구주소: [] }],
  };
  const 기록 = {};
  const { 구조체 } = await 살채우기({
    뼈대: 생활부분뼈대, 프로필, 매니페스트, 읽기,
