...
test('지정 요점을 매니페스트에서 찾아 뼈대에 붙인다', async () => {
  ...
  // 매니페스트는 과 단위 요점만 담아 요점 4의 본문을 못 가져오고 3과 전체 원칙으로
  // 대신하므로, 그 사실을 대체 로 표시해야 한다.
  assert.equal(뼈대.지정요점.대체, true);
});

test('요점 번호가 없는 지정 요점은 대체로 표시하지 않는다', async () => {
  const { 뼈대 } = await 뼈대만들기({
    배정: { ...시연배정, 지정요점: { 책: '랑제', 과: 3, 요점: null } }, 프로필, 매니페스트,
    설정: { apiKey: '', fetchImpl: async () => { throw new Error('불리면 안 된다'); } },
  });

  assert.equal(뼈대.지정요점.대체, false);
});
...
test('시연은 키가 있으면 AI 응답의 구간을 쓴다', async () => {
  ...
  assert.ok(뼈대.출처.some(x => x.제목 === '「랑제」 3과 친절'));
  // 설계 3절이 요구한 요점배치 를 버리지 않고 뼈대에 실어야 한다.
  assert.equal(뼈대.요점배치, '여는 인사');
});

test('성경낭독은 키가 없으면 낭독범위를 성구로 담는다', async () => {
  const 성경낭독배정 = {
    번호: 3, 제목: '성경 낭독', 절: '보물', 종류: '성경낭독', 시간초: 240,
    봉사형태: '', 설명: '렘 30:1-11.', 지정요점: null, 소제목: [], 묵상: '',
    낭독범위: '렘 30:1-11', 교재원문: '',
  };
  const { 뼈대, 생성 } = await 뼈대만들기({
    배정: 성경낭독배정, 프로필, 매니페스트, 설정: { apiKey: '', fetchImpl: async () => { throw new Error('x'); } },
  });

  assert.equal(생성.mode, 'fallback');
  assert.deepEqual(뼈대.성구, [{ 주소: '렘 30:1-11', 본문: '' }]);
  // 시연·성경낭독은 소제목이 없으므로 「소제목 하나를 다룬다」가 나가면 안 된다.
  assert.ok(뼈대.구간.every(x => !/소제목 하나를 다룬다/.test(x.목적)));
});

test('시연도 결정론적 뼈대에서는 본론 구간의 목적이 소제목을 말하지 않는다', async () => {
  const { 뼈대 } = await 뼈대만들기({
    배정: 시연배정, 프로필, 매니페스트, 설정: { apiKey: '', fetchImpl: async () => { throw new Error('x'); } },
  });
...
test('학생연설은 결정론적 뼈대에서 본론 구간의 목적이 소제목을 말한다', async () => {
  const 학생연설배정 = {
    번호: 5, 제목: '학생 연설', 절: '생활', 종류: '학생연설', 시간초: 300,
    봉사형태: '', 설명: '', 지정요점: null, 소제목: [], 묵상: '', 낭독범위: '', 교재원문: '',
  };
  const { 뼈대 } = await 뼈대만들기({
    배정: 학생연설배정, 프로필, 매니페스트, 설정: { apiKey: '', fetchImpl: async () => { throw new Error('x'); } },
  });

  assert.ok(뼈대.구간.some(x => x.목적 === '소제목 하나를 다룬다'));
});
