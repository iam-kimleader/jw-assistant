...
import { 기본설정, 시간보고, 분초표기, 이미포함됨 } from './talk-timing.mjs';
import { 인용으로감싸기 } from './talk-verses.mjs';
...
// 매니페스트가 조회일 없이 '조회일 미상' 문자열을 이미 넣어 주는 경우가 있어
// 그대로 감싸면 "(조회일 조회일 미상)" 으로 낱말이 겹친다.
function 조회일표기(조회일) {
  const 값 = String(조회일 ?? '').trim();
  return !값 || 값 === '조회일 미상' ? '조회일 미상' : `조회일 ${값}`;
}
...
// 지정요점붙이기(talk-outline.mjs)가 요점 번호의 본문 대신 과 전체의 요점으로
// 대신했을 때(대체: true) 그 사실을 밝힌다. 매니페스트가 과 단위 요점만 담기
// 때문에 생기는 대체이지 원고의 결함이 아니지만, 감춰서는 안 된다.
function 대체안내(지정요점) {
  return `이것은 「${지정요점.책}」 ${지정요점.과}과 전체의 원칙이다. 요점 ${지정요점.요점}의 `
    + '본문은 자동으로 가져오지 못했으므로 교재의 링크를 직접 확인하십시오.';
}
...
function 준비원고쓰기(구조체, 보고) {
  ...
  if (구조체.지정요점) {
    ...
    if (구조체.지정요점.대체) 줄.push('', 대체안내(구조체.지정요점));
  }
  ...
  if ((구조체.단락 ?? []).length) {
    ...
    for (const 단락 of 구조체.단락) {
      줄.push(`### ${단락.소제목}`, '');
      // 교재·개요에서 온 소제목은 출처가 명백하니 굳이 밝히지 않는다. AI가 지어낸
      // 소제목(생성)만 그렇다고 표시한다. 「출처는 생성다.」는 한국어가 아니다.
      if (단락.출처 === '생성') 줄.push('이 소제목은 교재가 준 것이 아니다.', '');
  ...
  if ((구조체.출처 ?? []).length) {
    줄.push('', '## 출처', '');
    for (const 출처 of 구조체.출처) 줄.push(`- ${출처.제목}. ${출처.url} (${조회일표기(출처.조회일)})`);
  }
...
function 낭독대본쓰기(구조체, 보고) {
  // 「두 사람이 각자 들고 읽는」 것은 시연뿐이다. 다른 종류는 혼자 낭독한다.
  const 안내 = 구조체.종류 === '시연'
    ? '두 사람이 각자 들고 읽는 용도다. 배경과 근거는 준비용 원고에 있다.'
    : '배경과 근거는 준비용 원고에 있다.';
  const 줄 = [...머리(구조체, 보고), '', 안내, ''];
  ...
  for (const 성구 of 구조체.성구 ?? []) {
    // 대사나 단락에 {{성구:…}} 자리표시자가 이미 로컬 본문으로 치환돼 있으면
    // 여기서 또 나열하면 같은 절을 두 번 읽게 된다.
    if (성구.본문 && !이미포함됨(성구.본문, 구조체)) {
      줄.push(`**${성구.주소}** ${인용으로감싸기(성구.본문)}`, '');
    }
  }
...
function 점검표쓰기(구조체, 보고) {
  ...
  if (구조체.지정요점) {
    ...
    if (구조체.지정요점.대체) 줄.push(대체안내(구조체.지정요점), '');
    if (구조체.요점배치) 줄.push(`뼈대 단계에서 정한 배치. ${구조체.요점배치}`, '');
  }

  const 대목 = [
    ...(구조체.대사 ?? []).filter(x => String(x.요점표시 ?? '').trim()).map(x => ({ 이름: `${x.구간} · ${x.화자}`, 요점표시: x.요점표시 })),
    ...(구조체.단락 ?? []).filter(x => String(x.요점표시 ?? '').trim()).map(x => ({ 이름: x.소제목, 요점표시: x.요점표시 })),
  ];
  if (!구조체.지정요점) {
    // 지정요점 이 애초에 없는 배정(예. 보물연설 대부분)은 점검할 지정 요점이 없는
    // 것이지 결함이 아니므로 경고 표시(⚠)를 쓰지 않는다.
    줄.push('이 배정에는 교재가 지정한 요점이 없습니다. 스스로 정한 목표를 여기에 적어 두십시오.');
  } else if (대목.length) {
    줄.push('| 대목 | 요점이 드러나는 방식 |', '| --- | --- |');
    for (const 항목 of 대목) 줄.push(`| ${항목.이름} | ${항목.요점표시} |`);
  } else {
