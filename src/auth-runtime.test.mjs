...
// 설정읽기 는 셋이 다 있어야 통과한다. 한 변수만 빼는 시험이 그 변수 때문에
// 던진 것임을 확실히 하려고 갖춘 상태를 한곳에 둔다.
const 갖춘환경 = {
...
test('APP_BASE_URL 끝의 빗금을 지운다', () => {
  const 설정 = 환경으로({ ...갖춘환경, APP_BASE_URL: 'https://example.com/' }, 설정읽기);
  assert.equal(설정.redirectUri, 'https://example.com/api/auth-callback');
});

test('Vercel 에서만 Secure 쿠키를 쓴다', () => {
  assert.equal(환경으로({ ...갖춘환경, VERCEL: '1' }, 설정읽기).보안, true);
  assert.equal(환경으로({ ...갖춘환경, VERCEL: undefined }, () => 설정읽기()).보안, false);
});

test('SESSION_SECRET 이 없거나 짧으면 설정읽기가 던진다', () => {
  assert.throws(
    () => 환경으로({ ...갖춘환경, SESSION_SECRET: undefined }, 설정읽기),
    /SESSION_SECRET/);
  assert.throws(
    () => 환경으로({ ...갖춘환경, SESSION_SECRET: 'x'.repeat(31) }, 설정읽기),
    /SESSION_SECRET/);
});

test('KAKAO_REST_API_KEY 가 없으면 설정읽기가 던진다', () => {
  assert.throws(
    () => 환경으로({ ...갖춘환경, KAKAO_REST_API_KEY: undefined }, 설정읽기),
    /KAKAO_REST_API_KEY/);
});

test('KAKAO_CLIENT_SECRET 이 없으면 설정읽기가 던진다', () => {
  assert.throws(
    () => 환경으로({ ...갖춘환경, KAKAO_CLIENT_SECRET: undefined }, 설정읽기),
    /KAKAO_CLIENT_SECRET/);
});
