...
// talk-outline.mjs 의 구간나누기 가 구간을 서론 / 본론 1..N / 결론 순서로 만들고
// 단락은 그 본론과 순서대로 대응한다(본론 N == 단락[N-1]). 구조체에 그 대응을
// 담는 새 필드는 없으므로 여기서 인덱스로 계산한다.
function 축약된대상(구조체, 순위) {
  ...
  const 남은단락 = (구조체.단락 ?? [])
    .map((단락, i) => {
      const 지목됨 = 지울것.has(단락.소제목) || 지울것.has(`본론 ${i + 1}`);
      if (!지목됨) return 단락;
      if (단락.출처 === '생성') return null; // 교재가 준 게 아니므로 통째로 뺀다
      // 교재·개요가 준 소제목·출처·성구주소는 지키고, 우리가 채운 칸만 비운다.
      return { ...단락, 요점: [], 예화: '', 적용: '' };
    })
    .filter(Boolean);
