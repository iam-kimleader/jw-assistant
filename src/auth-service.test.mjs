...
test('설정을 저장해도 닉네임이 지워지지 않는다', async () => {
  const 저장소 = 가짜저장소({
    'users/777/profile.json': { 회원번호: '777', 닉네임: '동언', 첫로그인때: '2026-08-27T00:00:00.000Z' },
  });
  await 만들기(저장소).설정쓰기('777', { 임명: '장로', 연령: 40 });
  const 기록 = 저장소.담긴것.get('users/777/profile.json');
  assert.equal(기록.닉네임, '동언');
  // 로그인완료 가 실제로 넣는 이름이다. 아무 이름이나 쓰면 진짜 필드가 지워져도 시험이 통과한다.
  assert.equal(기록.첫로그인때, '2026-08-27T00:00:00.000Z');
