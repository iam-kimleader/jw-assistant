...
const 스키마 = {
  ...
  properties: {
    ...
    단락: { type: 'array', items: { type: 'object', properties: {
      ...
      요점표시: { type: 'string' },
    }, required: ['소제목', '요점', '예화', '적용', '성구주소', '요점표시'], additionalProperties: false } },
...
function 지시만들기(뼈대, 되풀이) {
  const 줄 = [
    ...
    '지정 요점이 드러나는 대사나 단락에는 요점표시를 채우십시오. 나머지의 요점표시는 빈 문자열로 두십시오.',
    '축약순서에는 시간을 넘길 때 먼저 뺄 곳을 순위대로 담으십시오. 대상은 구간 이름 또는 소제목 문장으로 쓰고, 반드시 주어진 구간과 소제목 가운데 하나를 글자 그대로 쓰십시오.',
...
function 소제목지키기(뼈대, 받은단락) {
  if (!(뼈대.단락 ?? []).length) {
    return (받은단락 ?? []).map(단락 => ({ ...단락, 출처: '생성', 요점표시: 단락.요점표시 ?? '' }));
  }
...
export async function 살채우기({ 뼈대, 프로필, 읽기, 설정 = {} }) {
  const 자료 = {
  ...
  let 마지막경고 = '';
  // 재요청은 시간이 두 배로 든다. 서버리스 함수 제한 안에 못 들어가면 폴백조차 못 돌려주므로,
