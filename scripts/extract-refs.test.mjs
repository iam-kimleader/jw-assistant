...
const lines = skip ? [] : splitLines(readFileSync(FILE, 'utf8')).filter(Boolean);
...
test('66개 파일 모든 줄이 장:절 과 참조 목록으로 이루어진다', { skip }, () => {
  ...
  for (const file of files) {
    const bookLines = splitLines(readFileSync(`${REFS_DIR}/${file}`, 'utf8')).filter(Boolean);
...
test('에스라 4:8 은 장을 넘어가는 범위 참조를 정확한 장:절로 표기한다', { skip }, () => {
  const ezraLines = splitLines(readFileSync(`${REFS_DIR}/15-에스라.tsv`, 'utf8')).filter(Boolean);
...
test('열왕기상 9:10 은 장을 넘어가는 범위 참조를 정확한 장:절로 표기한다', { skip }, () => {
  const kingsLines = splitLines(readFileSync(`${REFS_DIR}/11-열왕기상.tsv`, 'utf8')).filter(Boolean);
...
test('역대기상 13:5 는 장을 넘어가는 범위 참조를 정확한 장:절로 표기한다', { skip }, () => {
  const chroniclesLines = splitLines(readFileSync(`${REFS_DIR}/13-역대기상.tsv`, 'utf8')).filter(Boolean);
...
test('다니엘 2:4 는 장을 넘어가는 범위 참조를 정확한 장:절로 표기한다', { skip }, () => {
  const danielLines = splitLines(readFileSync(`${REFS_DIR}/27-다니엘.tsv`, 'utf8')).filter(Boolean);
