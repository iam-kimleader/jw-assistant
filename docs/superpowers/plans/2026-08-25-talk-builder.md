...
- Produces:
  ...
  - `찾기(매니페스트, 책, 과)` → 매니페스트에 담긴 과 객체 그대로, 또는 `null`. 과 객체는 `{ 번호, 제목, 요점, 원칙, docid }` 다. 「랑제」는 `요점`이 비고 `원칙`이 차므로 과제 9가 둘을 함께 본다. 필드를 골라 새 객체를 만들지 않는다.
...
- [ ] **Step 1: 픽스처를 실물에서 잘라 만든다**

**픽스처를 손으로 지어내지 마라.** 과제 1이 그렇게 했다가 실물과 어긋나 되돌렸다.
요소가 존재하는 것과 감싸는 구조·속성이 같은 것은 다른 문제이며, 후자는 실물을
잘라내야만 보장된다.

실물 교재가 저장소에 캐시되어 있다. `.cache/wol/probe-lm-0824.html` (docid 202026248,
2026년 8월 24-30일 주)이다. 없으면 이렇게 받는다.

```
node --no-warnings -e "import('./src/wol-fetch.mjs').then(m => m.fetchCached('https://wol.jw.org/ko/wol/d/r8/lp-ko/202026248','probe-lm-0824.html'))"
```

이 파일에서 `tests/fixtures/교재-배정.html` 을 잘라낸다. 규칙은 셋이다.

1. **남길 항목은 일곱이다.** 1번 보물 연설, 2번 영적 보물 찾기, 3번 성경 낭독,
   4번 대화 시작하기, 6번 연설, 7번 그리스도인 생활 항목, 9번 회중 성서 연구다.
   5번과 8번은 4번·7번과 같은 모양이라 빼도 된다.
2. **`h2` 절 제목 셋(`성경에 담긴 보물`, `야외 봉사에 힘쓰십시오`, `그리스도인 생활`)과
   `article` 여는 태그를 반드시 남긴다.** 절 판정이 그것에 달려 있다.
3. **태그·class·중첩 구조를 글자 그대로 보존하라.** `(N분)` 을 감싸는 `div` 의 class,
   앵커의 `class="b"` 와 `data-bid`, `figure`/`img`, `묵상해 볼 점` 의 `span`/`strong`
   중첩을 손대지 마라. 이것이 이 단계의 요점이다.

7번 항목의 본문 문단은 한두 개만 남기고 나머지는 잘라도 된다. 잘라낸 자리에 다른
문장을 채워 넣지 마라. 이 저장소는 출판물 본문을 담지 않는 규칙이 있고 픽스처는
발췌로 유지한다.

잘라낸 뒤 아래 함정 다섯이 픽스처에 **실제로 남아 있는지** 확인하라. 하나라도 잘려
나갔으면 그 항목을 다시 넣어라. 함정은 이것이다.
...
아래는 **참고용 구조 스케치**다. 이 HTML 을 그대로 픽스처로 쓰지 마라. 실물에서
잘라낸 것과 이 스케치를 대조해, 남겨야 할 항목이 빠지지 않았는지 확인하는 용도다.
실물에는 여기 없는 `div` 감싸개와 class 와 `data-bid` 가 더 있으며, 그것들을
살려 두는 것이 이 단계의 목적이다.
...
픽스처가 아니라 **캐시된 실물 교재**로 파서를 돌린다.

```
node --no-warnings -e "
import('./src/talk-assignments.mjs').then(async m => {
  const { readFileSync } = await import('node:fs');
  const 배정 = m.parseTalkAssignments(readFileSync('.cache/wol/probe-lm-0824.html','utf8'));
  for (const x of 배정)
    console.log(x.번호, x.절, x.종류, x.시간초 + '초', JSON.stringify(x.지정요점), '소제목' + x.소제목.length);
});"
```
...
```
1 보물 보물연설 600초 null 소제목3
2 보물 연설아님 600초 null 소제목0
3 보물 성경낭독 240초 {"책":"읽가","과":2,"요점":null} 소제목0
4 야외봉사 시연 240초 {"책":"랑제","과":3,"요점":4} 소제목0
5 야외봉사 시연 180초 {"책":"랑제","과":1,"요점":5} 소제목0
6 야외봉사 학생연설 300초 {"책":"읽가","과":1,"요점":null} 소제목0
7 생활 생활부분 600초 null 소제목0
8 생활 생활부분 300초 null 소제목0
9 생활 연설아님 1800초 null 소제목0
```

**이 출력을 보고서에 그대로 붙여라.** 어긋나면 픽스처가 아니라 파서를 고쳐라.
과제 1이 정확히 이 검증이 없어서 결함을 안고 리뷰를 통과했다.

8번은 교재 원문이 `(5분) 봉사 감독자의 연설.` 이라 `종류판정` 의 마지막 줄에서
`/토의|연설/` 에 걸려 `학생연설` 로 나올 수 있다. 그렇게 나오면 그것은 결함이다.
`절 === '생활'` 검사가 먼저 오므로 `생활부분` 이 맞다. 실제 출력으로 확인하라.
