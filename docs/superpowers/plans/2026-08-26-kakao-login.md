...
## 2026-08-27 — 초대 코드 게이트를 없앤다

형제의 결정이다. 카카오 로그인만으로 충분하다. 비용 노출은 이미 알고
받아들였다. 이 계획으로 만든 `초대확인`·`/invite`·`api/auth-invite.js`·
`신원쿠키이름`·`INVITE_CODE`를 모두 없앴다. `users/{회원번호}/profile.json`은
남긴다 — 3~4단계가 쓸 닉네임·첫 로그인 시각 부기이지만, 더 이상 입장을 정하지
않으므로 쓰기 실패를 조용히 넘기고 로그인을 막지 않는다. `kakao-login` 브랜치
위에서 바로 고쳤다. 설계 문서를 함께 고쳤다.
